...
public abstract class Store extends Observable {
    ...
    void handleAction(Action action) {
        if (action instanceof ErrorAction) {
            ...
            if (onError(errorAction.getAction(), errorAction.getThrowable())) {
                postError(errorAction.getAction());
            }
        } else {
    ...
    private void postChange(Action action) {
        Logger.logPostStoreChange(getClass().getSimpleName(), action.getType());
        notifyChange(action);
    }

    private void postError(Action action) {
        Logger.logPostStoreError(getClass().getSimpleName(), action.getType());
        notifyError(action);
    }
}
